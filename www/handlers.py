...
async def authenticate(*, email, passwd):
    ...
    if user.passwd != sha1.hexdigest():
        raise APIValueError('passwd', 'Invalid passwoed.')
...
async def api_comments(*, page='1'):
    ...
    comments = await Comment.findAll(orderBy='created_at_desc', limit=(p.offset, p.limit))
...
async def api_update_blog(id, request, *, name, summary, content):
    ...
    if not summary or not summary.strip():
        raise APIValueError('summary', 'summary cannot be empty.')
    if not content or not content.strip():
        raise APIValueError('content', 'conent cannot be empty.')
...
async def api_delete_users(id, request):
    ...
    if comments:
        for comment in comments:
            id = comment.id
            c = await Comment.find(id)
            c.user_name = c.user_name + '(该用户已经删除)'
            await c.update()
    id = id_buff
    return dict(id=id)
