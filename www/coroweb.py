...
def has_request_arg(fn):
    ...
    for name, param in params.items():
        ...
        if found and (param.kind != inspect.Parameter.VAR_POSITIONAL and param.kind
                                 != inspect.Parameter.KEYWORD_ONLY and param.kind
                                 != inspect.Parameter.VAR_KEYWORD):
            raise ValueError('request parameter must be the last named parameter in functino: %s%s' % (fn.__name__, str(sig)))
    return found


# 定义RequestHandler从URL函数中分析其需要接受的函数
class RequestHandler(object):
    ...
    async def __call__(self, request):
        ...
        try:
            ...
            return dict(error=e, data=e.data, message=e.message)
